@php
    ...
    $home = is_array($d['home'] ?? null) ? $d['home'] : [];
    ...
    <tr>
        <td style="padding:26px 22px;border-radius:18px;text-align:center;color:#ffffff;background:linear-gradient(135deg,#0a1d37 0%,#14532d 52%,#0d2944 100%);">
            <p style="margin:0 0 12px;display:inline-block;padding:7px 14px;border-radius:999px;background:linear-gradient(135deg,#22c55e,#15803d);font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#ffffff;">
                {{ $d['eyebrow'] ?? 'Ready to Go' }}
            </p>
            <p style="margin:0 0 10px;font-size:28px;line-height:1;">✅</p>
            <h2 style="margin:0 0 8px;font-size:22px;line-height:1.3;font-weight:800;color:#ffffff;">{{ $d['headline'] ?? 'Final Checklist' }}</h2>
            @if(!empty($d['tagline']))
                <p style="margin:0 0 8px;font-size:15px;font-weight:700;color:rgba(255,255,255,0.92);">{{ $d['tagline'] }}</p>
            @endif
            @if(!empty($d['intro']))
    ...
    <tr>
        <td style="padding:18px;">
            ...
            @if(!empty($d['report_time']))
                <p style="margin:0;font-size:14px;font-weight:700;color:#16a34a;"><strong>⏰ Reporting Time:</strong> {{ $d['report_time'] }}</p>
            @endif
        </td>
    </tr>
</table>
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:14px 0 0;border:1px solid #bbf7d0;border-radius:16px;background:#f0fdf4;">
    <tr>
        <td style="padding:18px;">
            <h3 style="margin:0 0 12px;font-size:15px;font-weight:800;color:#0a1d37;">🏠 {{ $d['home_title'] ?? 'Before You Leave Home' }}</h3>
            @include('emails.layouts._checklist', ['items' => $home])
        </td>
    </tr>
</table>
@endif
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:14px 0 0;border:1px solid #e5e7eb;border-radius:16px;background:#ffffff;">
    <tr>
        <td style="padding:18px;">
            <h3 style="margin:0 0 12px;font-size:15px;font-weight:800;color:#0a1d37;">ℹ️ {{ $d['info_title'] ?? 'Important Information' }}</h3>
