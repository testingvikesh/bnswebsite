@php
    ...
    $early = is_array($d['early'] ?? null) ? $d['early'] : [];
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;">
    <tr>
        <td style="padding:26px 22px;border-radius:18px;text-align:center;color:#ffffff;background:linear-gradient(135deg,#0a1d37 0%,#b45309 48%,#0d2944 100%);">
            <p style="margin:0 0 12px;display:inline-block;padding:7px 14px;border-radius:999px;background:linear-gradient(135deg,#fd8a2e,#fd6e01);font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#ffffff;">
                {{ $d['eyebrow'] ?? 'Final Day Prep' }}
            </p>
            <p style="margin:0 0 10px;font-size:28px;line-height:1;">☀</p>
            <h2 style="margin:0 0 8px;font-size:22px;line-height:1.3;font-weight:800;color:#ffffff;">🌟 {{ $d['headline'] ?? 'Tomorrow Is The Day!' }}</h2>
            @if(!empty($d['tagline']))
                <p style="margin:0 0 8px;font-size:14px;font-weight:700;color:rgba(255,255,255,0.92);">{{ $d['tagline'] }}</p>
            @endif
            @if(!empty($d['greeting']))
                <p style="margin:0 0 8px;font-size:15px;font-weight:700;color:#ffffff;">{{ $d['greeting'] }}</p>
            @endif
            @if(!empty($d['thanks']))
                <p style="margin:0 0 6px;font-size:14px;line-height:1.65;color:rgba(255,255,255,0.88);">{{ $d['thanks'] }}</p>
            @endif
            @if(!empty($d['welcome']))
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:14px 0 0;border:1px solid #ffedd5;border-radius:16px;background:#fff7ed;">
    <tr>
        <td style="padding:18px;">
            <h3 style="margin:0 0 12px;font-size:15px;font-weight:800;color:#0a1d37;">⏰ {{ $d['early_title'] ?? 'Please Arrive Early' }}</h3>
            @include('emails.layouts._checklist', ['items' => $early])
            @if(!empty($d['seats']))
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:14px 0 0;border-collapse:separate;">
    <tr>
        @if($dress !== [])
            <td width="{{ $bring !== [] ? '50%' : '100%' }}" valign="top" style="padding:0 {{ $bring !== [] ? '6px' : '0' }} 0 0;">
        ...
        @endif
        @if($bring !== [])
            <td width="{{ $dress !== [] ? '50%' : '100%' }}" valign="top" style="padding:0 0 0 {{ $dress !== [] ? '6px' : '0' }};">
                <table role="presentation" width="100%" style="border:1px solid #e5e7eb;border-radius:16px;background:#ffffff;">
                    <tr>
                        <td style="padding:16px;">
                            <h3 style="margin:0 0 12px;font-size:14px;font-weight:800;color:#0a1d37;">🎒 {{ $d['bring_title'] ?? 'Please Bring' }}</h3>
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:14px 0 0;border:1px solid #e5e7eb;border-radius:16px;background:#ffffff;">
    <tr>
        <td style="padding:18px;">
            <h3 style="margin:0 0 12px;font-size:15px;font-weight:800;color:#0a1d37;">🎯 {{ $d['learn_title'] ?? 'What Will You Learn?' }}</h3>
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:14px 0 0;border:1px solid #e5e7eb;border-radius:16px;background:#f8fafc;">
    <tr>
        <td style="padding:18px;text-align:center;">
            ...
            @endif
            @if(!empty($d['final_welcome']))
                <p style="margin:0 0 8px;font-size:14px;line-height:1.65;color:#334155;">{{ $d['final_welcome'] }}</p>
            @endif
            @if(!empty($d['closing']))
                <p style="margin:0 0 8px;font-size:14px;font-style:italic;color:#475569;">{{ $d['closing'] }}</p>
            @endif
            @if(!empty($d['brand']))
