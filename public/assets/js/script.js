(function ($) {
  ...
  if ($(".video-popup").length) {
    $(".video-popup").magnificPopup({
      ...
      iframe: {
        markup: '<div class="mfp-iframe-scaler"><div class="mfp-close"></div><iframe class="mfp-iframe" frameborder="0" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin"></iframe></div>',
        patterns: {
          youtube: {
            index: "youtube.com/",
            id: function (url) {
              var m = url.match(/(?:embed\/|watch\?v=|shorts\/|youtu\.be\/)([A-Za-z0-9_-]{11})/);
              return m ? m[1] : null;
            },
  ...
  if ($(".mobile-nav__container .main-menu__list").length) {
    ...
    dropdownAnchor.each(function () {
      ...
      self.on("click", function (e) {
        e.preventDefault();
        let $anchor = $(this);
        let $btn = $anchor.find("button");
        $btn.toggleClass("expanded");
        $anchor.toggleClass("expanded");
        $anchor.parent().children("ul").slideToggle();
      });
    });
  }
