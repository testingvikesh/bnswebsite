...
use App\Support\TeamMemberProfiles;
...
class TeamPageService
{
    ...
    private function defaultMembers(string $category): Collection
    {
        ...
        return collect($config[$key]['members'] ?? [])->map(function (array $item, int $index) use ($category) {
    ...
    private function resolveProfile(array $item): ?string
    {
        if (! empty($item['profile'])) {
            return (string) $item['profile'];
        }

        if (! empty($item['profile_data']) && is_array($item['profile_data'])) {
            return TeamMemberProfiles::render($item['profile_data']);
        }
