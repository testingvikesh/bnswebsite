...
class VisitingExpertFacultyController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $faculty->getCollection()->each(function (VisitingExpertFaculty $member) {
            if ($member->migratePhotoToPublicUploads()) {
                $member->refresh();
            }
        });
    ...
    {
        $visiting_faculty->migratePhotoToPublicUploads();
        $visiting_faculty->refresh();
    ...
    private function storePhoto($file): string
    {
        ...
        $filename = 'faculty-'.time().'-'.Str::lower(Str::random(6)).'.'.strtolower($extension);
        $file->move($directory, $filename);

        return 'uploads/visiting-faculty/'.$filename;
    }
}
